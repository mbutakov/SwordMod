...
public class RenderItemSwordCyan implements IItemRenderer {

	
	public RenderItemSwordCyan() {
	}
	...
	public void renderItem(ItemRenderType type, ItemStack is, Object... data) {
			...
			if(Cplayer.getHeldItem() != null) {
				if(Cplayer.getHeldItem().getItem() instanceof Sword) {
					if(Cplayer.ticksExisted % 2  == 0) {
						Main.proxy.spawnEffectSword(Cplayer.worldObj, Cplayer, (double)((float)posX), (double)((float)posY + Cplayer.eyeHeight - 1.1f), (double)((float)posZ ), (double)((float)posX), (double)((float)posY + Cplayer.eyeHeight - 1), (double)((float)posZ), 0.25F,((Sword)Cplayer.getHeldItem().getItem()).getColorEffect(), true, -1f,false);
					}
				}
			}
		}
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(mbResourceLocation.CyanSwordTex);
		mbResourceLocation.CyanSword.renderAll();
		GL11.glPopMatrix();
		GL11.glPopMatrix();
	}
}